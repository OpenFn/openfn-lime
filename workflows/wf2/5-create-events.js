...
fn(state => {
  ...
  state.encountersMapping = state.encounters.map(data => {
    ...
    const valueUuids = data.obs.map(o => o.value.uuid);
    const dataValues = optsMap
      .filter(o => valueUuids.includes(o['value.uuid - External ID']))
      .map(o => {
        return {
          dataElement: '', //col B,
          value: o['DHIS2 Option UID'],
        };
      });
...
//     },
//   }) //TODO: Add query parameter '/events?dataElementIdScheme=UID'
// );
